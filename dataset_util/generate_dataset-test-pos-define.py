...
# This scrip is to check how is the coordinate defination compared to isaac sim
...
def get_obs_from_random_pose(sim, agent, cfg, pos, yaw, offset):
    agent_state = agent.get_state()
    # 固定 x 值，只改变 y 值
    pos_offset = np.array([0.0, offset, 0.0])  # 只改变 y 值，增加 
    rot = np.array([0.0, 0.0, 0.0])  # 固定旋转角度
    agent_state.position = pos + pos_offset
    agent_state.rotation = R.from_euler(
        "xyz", rot + np.array([0.0, 0.0, 0.0])
    ).as_quat()
                    ...
                    for yaw in np.linspace(
                        ...
                        for i in range(2):
                            ...
                            obs, agent_state = get_obs_from_random_pose(
                                sim, agent, sampling_cfg, point, yaw, offest
                            )
                            offest += 1.0
                            meta = create_meta_and_save(
                                sim, agent, sim_settings, obs, floor_idx, i, tmp_path
                            )
                            metas.append(meta)
                            
                            # 打印位置信息
                            print(f"Image {i} position:")
                            print(f"x: {agent_state.position[0]:.2f}")
                            print(f"y: {agent_state.position[1]:.2f}")
                            print(f"z: {agent_state.position[2]:.2f}")
                            print(f"quaternion: w={agent_state.rotation.w:.2f}, x={agent_state.rotation.x:.2f}, y={agent_state.rotation.y:.2f}, z={agent_state.rotation.z:.2f}")
                            print("---")
